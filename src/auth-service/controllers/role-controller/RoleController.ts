...
import Permission from "@/auth-service/models/Permission";
import PermissionRole from "@/auth-service/models/junction_models/PermissionRole";
import UserController from "../user-controller/UserController";
import UserRole from "@/auth-service/models/junction_models/UserRole";
...
export default class RoleController {
    ...
    try {
      ...
      if (e instanceof UniqueConstraintError) {
        if (e.errors[0].path === "name") {
          throw CreateError("ROLE_NAME_MUST_BE_UNIQE");
        }
      }
      throw e;
    }
  ...
  public static async DeleteRolesByIds(ids: string[]): Promise<void> {
    try {
      await Promise.all(
        ids.map(async (id) => {
          if (this.staticRoleIdList.includes(id)) {
            throw CreateError("ROLE_DELETE_RESTRICTION");
          }
      ...
      if (e instanceof DatabaseError) {
        throw CreateError("UUID_SYNTAX_ERROR");
      }
      throw e;
    }
  }
  ...
  public static async ReadRoleById(id: string): Promise<Role> {
    try {
      ...
      if (!role) {
        throw CreateError("ROLE_NOT_FOUND");
      }
      return role;
    } catch (e) {
      if (e instanceof DatabaseError) {
  ...
  public static async AddPermissionToRole(
    ...
    try {
      ...
      if (e instanceof ForeignKeyConstraintError) {
        CreateError("PERMISSION_NOT_FOUND");
      }
      throw e;
    }
  }
  ...
  public static async AddRoleToUser(
    ...
    try {
      if (
        ...
        throw CreateError("ROLE_OF_ADMIN_NOT_CHANGEABLE");
      }
